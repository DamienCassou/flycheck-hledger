...
This package is a [[https://www.flycheck.org/en/latest/][flycheck]] checker for [[https://hledger.org][hledger]] files. It
supports hledger 1.40 and newer, including hledger 2 (and partially supports hledger 1.26+). If the package
doesn't work for your version of hledger, try to update it and [[https://github.com/DamienCassou/flycheck-hledger/issues/new][open an
issue]] if it still doesn't work.
...
When ~flycheck-mode~ is enabled in a journal buffer, hledger's ~check~
command is run on the file, and any problems are highlighted. The
checker always runs hledger with the ~--auto~ flag, so any auto posting
rules are active.

~C-c ! l~ lists any problems in the journal.
...
(Emacs's general ~C-x `~ also steps through problems, and
~C-u C-x `~ restarts from the first one; but when several files are
open, these will keep jumping back to the file where you first used them.)
